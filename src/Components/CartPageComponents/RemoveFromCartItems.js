...
const remove = async (token, id) => {
    try {
        // const response = await fetch('http://localhost:8000/user/update/cart/remove', {
        const response = await fetch('https://server.bitem.in/user/update/cart/remove', {
            method: 'POST',
...
export default function RemoveFromCartItems({user, setUser, id, isUpdating, setIsUpdating, productBeingUpdated, setProductBeingUpdated}) {
